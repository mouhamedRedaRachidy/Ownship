...
export function useCategorySettings(): CategorySetting[] {
  ...
  const merged = useMemo(() => {
    const overrides = ((data?.settings as { categories?: CategorySetting[] } | undefined)?.categories as CategorySetting[] | undefined) ?? [];
    ...
    return sorted;
  }, [(data?.settings as { categories?: CategorySetting[] } | undefined)?.categories]);
