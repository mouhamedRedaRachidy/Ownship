...
//
// This version only intercepts the key when the slash menu actually has
// visible items (cmdk renders `[cmdk-item][data-selected="true"]` only
// while the menu is open AND has matches).
const handleCommandNavigation = (event: KeyboardEvent): true | undefined => {
  if (!['ArrowUp', 'ArrowDown', 'Enter'].includes(event.key)) return undefined;
  const slash = document.querySelector('#slash-command');
  if (!slash) return undefined;
  const hasVisibleItems = slash.querySelector('[cmdk-item]') !== null;
  return hasVisibleItems ? true : undefined;
};
